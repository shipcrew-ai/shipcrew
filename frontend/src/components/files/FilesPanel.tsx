...
export function FilesPanel() {
  ...
  if (!filesPanelOpen) return null;

  const fileCount = countFiles(tree);

  return (
    <div className="w-[520px] min-w-[400px] border-l border-slack-border bg-slack-bg flex flex-col">
      {/* Header */}
      <div className="h-12 flex items-center border-b border-slack-border px-4 gap-3 flex-shrink-0">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          {fileCount > 0 && (
            <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-slack-hover text-slack-muted">
              {fileCount}
            </span>
          )}
        </div>
      ...
      <div className="flex-1 flex min-h-0">
        {/* Tree sidebar */}
        <div className="w-48 min-w-[160px] border-r border-slack-border overflow-y-auto flex-shrink-0">
